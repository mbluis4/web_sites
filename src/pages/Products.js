...
const Products = () => {
  const productList = productData.map((item) => (
    <Link key={item.id} to={`/products/${item.id}`}>
      ...
      items-center mr-3 justify-center w-52 bg-white py-3 mb-4 h-52"
      >
  ...
  return (
    <>
      ...
      md:-mt-10 md:text-4xl text-slate-500"
      >
        Productos
      </h1>
      <div
        className="flex flex-col sm:flex-row md:-mt-10 sm:justify-around 
      items-center sm:flex-wrap self-center"
      >
        {productList}
      </div>
    </>
